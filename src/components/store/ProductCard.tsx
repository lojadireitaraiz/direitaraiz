...
export function ProductCard({
  ...
  return <Link to={`/produto/${node.handle}`} className="product-card group">
      <div className="product-card-image">
        {imageUrl ? <img src={imageUrl} alt={node.images.edges[0]?.node.altText || node.title} loading="lazy" /> : <div className="w-full h-full bg-muted flex items-center justify-center text-muted-foreground">
            Sem imagem
          </div>}
        
        {discount > 0 && <span className="product-card-badge bg-secondary">
            {discount}% OFF
          </span>}
